...
class NonlinearController(object):
    ...
    def lateral_position_control(self, local_position_cmd, local_velocity_cmd, local_position, local_velocity,
        ...
        p = position_err * self.k_p_lat
        d = vel_err * self.k_d_lat

        acceleration_cmd = p + d + acceleration_ff
    ...
    def altitude_control(self, altitude_cmd, vertical_velocity_cmd, altitude, vertical_velocity, attitude, acceleration_ff=0.0):
        ...
        p = self.z_k_p * alt_err
        d = self.z_k_d * vel_err

        vert_acc_cmd = p + d + acceleration_ff
        
        b_z = np.cos(attitude[0]) * np.cos(attitude[1]) 

        c = DRONE_MASS_KG * vert_acc_cmd / b_z 
    ...
    def body_rate_control(self, body_rate_cmd, body_rate):
        ...
        kp_rate = np.array([self.k_p_p, self.k_p_q, self.k_p_r])


        moment_cmd = MOI * np.multiply(kp_rate, body_rate_err)

        moment_value = np.linalg.norm(moment_cmd)

        if moment_value > MAX_TORQUE:
            moment_cmd = moment_cmd*MAX_TORQUE/moment_value
    ...
    def yaw_control(self, yaw_cmd, yaw):
        ...
        if err_yaw > np.pi:
            err_yaw = err_yaw - 2.0*np.pi
        elif err_yaw < -np.pi:
            err_yaw = err_yaw + 2.0*np.pi

        r_c = self.k_p_yaw * err_yaw
